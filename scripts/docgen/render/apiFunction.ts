...
export function renderApiFunction(options: {
  ...
  const errorIds = resolveErrorData({
    dataLookup,
    id: `fhub!${module}.${displayName}.ErrorType:type`,
  })
  if (errorIds.size)
    content.push(
      renderErrors({
...
function renderImports(options: {
  ...
  const content = [
    ...
    [
      ':::code-group',
      '```ts [Named]',
      `import { ${module} } from 'fhub${entrypoint ? `/${entrypoint}` : ''}'`,
      '```',
      '```ts [Entrypoint]',
      `import * as ${module} from 'fhub/${entrypoint ? `${entrypoint}/` : ''}${module}'`,
      '```',
      ':::',
    ].join('\n'),
  ]
...
function renderErrors(options: {
  ...
  const typeRegex = /^fhub!(?<type>.+):type/
  const errorType =
...
function resolveErrorData(options: {
  ...
  for (const reference of errorData.references) {
    const nextErrorData =
      dataLookup[`fhub!${reference.text}:type`] ??
...
function getTypeLink(options: {
  ...
  const data = (() => {
    ...
    // if (dataLookup[`fhub!${type.type}:type`])
    //   return dataLookup[`fhub!${type.type}:type`]
...
function expandInlineType(options: {
  ...
  const expandRegex = /^fhub!(?<type>.+)(_2):type/
  if (
  ...
  } else if (dataLookup[`fhub!${type.type}:type`]) return type.type
  return type.type
}

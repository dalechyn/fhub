...
const fileName = './scripts/docgen/fhub.api.json'
...
for (const path of Object.values(exports.src)) {
  if (!path.endsWith('index.ts') && !path.endsWith('Actions.ts')) continue
...
const apiEntryPoint = apiPackage.members.find(
  (x) =>
    ...
    x.canonicalReference.toString() === 'fhub!',
  ...
  for (const { category, items } of namespace.categories) {
    ...
    for (const item of items) {
      const description = item.docComment?.summary
        .split('\n\n')[0]
        ?.replace('\n', ' ')
      content += `<tr><td className="vocs_TableCell"><a className="vocs_Anchor vocs_Link vocs_Link_accent_underlined" href="${item.name !== 'Actions' ? item.sidebarItem.link : '/actions'}">${item.name}</a></td><td className="vocs_TableCell">${description}</td></tr>\n`
    }
  }
  content += '</tbody>\n'
  content += '</table>\n'
